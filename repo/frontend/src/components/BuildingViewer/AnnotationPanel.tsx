...
export default function AnnotationPanel({
  ...
  const handleSave = async () => {
    setIsSaving(true)
    try {
      const response = await caseApi.updateAnnotation(
        annotation.fire_case_id,
        annotation.id,
        { label, description }
      )
      onUpdate(response.data)
    } catch (error) {
      console.warn('Annotation update API unavailable, applying locally:', error)
      onUpdate({ ...annotation, label, description })
    } finally {
      setIsSaving(false)
      setIsEditing(false)
    }
